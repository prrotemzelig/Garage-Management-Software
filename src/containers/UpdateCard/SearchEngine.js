...
setTheStates = () => {
        this.props.onChangeModalHistory(false); 
  ...
  onChildClicked(data,num,mode) {
    this.setState({click : true,modal: true,ticketNumber:num });
    this.modalOpen();
    this.props.onChangeModalHistory(mode);   
  }
     ...
     this.state.carNumber= this.props.value;
          if(this.state.carNumber!== "" ){
            if ( !this.props.loading ) { // if it not true - if we not loading
              for(var i=0;i<this.props.cards.length;i++){
                if(this.state.carNumber === this.props.cards[i].cardData.licenseNumber ){
                allMatchingCards.push(this.props.cards[i]);
                }
              }
            ...
            // if(this.state.found && {cards}!==null){
            // }
            // else{
            //   this.state.found=false;
            // }
      ...
      if(!this.state.found && this.state.carNumber!=='' ){
        ...
        return ( 
          ...
          <div style={ this.props.backgroundColor === 'light' ? {direction: "rtl",fontFamily: "Alef Hebrew" }:{ direction: "rtl",fontFamily: "Alef Hebrew", color: "white"}}>
            <h5 style={{textAlignLast: "center", marginTop: "40px",fontSize: "22px" ,fontWeight: "bold"}}>לא נמצאה היסטוריה עבור רכב זה </h5></div> 
      );
      }
      ...
      return (
        <div class="table-wrapper" style={this.props.backgroundColor=== 'light' ?
            ...
            <table class="table table-bordered" style={{marginBottom: "1px",direction: "rtl",fontFamily: "Alef Hebrew",tableLayout: "fixed",marginTop: "30px"}} >
                <thead>
...
const mapDispatchToProps = dispatch => { // for this to work we need to connect this constant "mapDispatchToProps" with our component 
  return {
      ...
      onChangeModalHistory: (mode)=>dispatch(actions.changeModalHistory(mode))
  };
};
